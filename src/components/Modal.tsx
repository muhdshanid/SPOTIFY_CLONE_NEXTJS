...
const Modal = ({title, onChange, isOpen, children, desc}: ModalProps) => {
  return (
   ...
   >
    <Dialog.Portal>
        ...
        <Dialog.Content className='fixed drop-shadow-md border border-neutral-700 top-[50%]
        ...
        translate-x-[-50%] translate-y-[-50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none'>
            ...
            <Dialog.Description className='mb-5 text-sm leading-none text-center'>
                {desc}
            </Dialog.Description>
            <div>
                {children}
            ...
            <Dialog.Close asChild>
                ...
                inline-flex h-[25px] w-[25px] appearance-none items-center justify-center rounded-full
                focus:outline-none'>
                    <IoMdClose/>
                </button>
            </Dialog.Close>
